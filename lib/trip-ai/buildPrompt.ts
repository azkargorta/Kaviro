  ...
  const modeInstructions: Record<TripAiMode, string> = {
    ...
    planning:
      [
        ...
        "- Si el CONTEXTO incluye «RAÍLES PLAN POR DÍA» con actividades existentes, no emitas KAVIRO_ITINERARY para sustituir todo salvo petición explícita de reemplazo; prioriza KAVIRO_DIFF por día o KAVIRO_DAYPLAN.",
      ].join("\n"),
    ...
    optimizer: [
      ...
      "Trabaja por DÍA (fecha concreta): si detectas horas muertas en el contexto, pregunta si quieren añadir actividades ese día antes de proponer creates.",
      "Si el usuario pide más planes o rellenar un día, añade con create_activity sin duplicar lo ya listado ese mismo día.",
      "Permite la misma atracción en días distintos (recorridos multi-día o ver un día y subir otro).",
    ...
    actions: [
      ...
      "Ancla los cambios a un día concreto cuando el usuario hable de «ese día» o una fecha.",
      "Al añadir actividades, evita duplicar el mismo sitio el mismo día; varios días para el mismo lugar sí es válido (p. ej. cataratas o parque).",
    ...
    day_planner:
      [
        ...
        "- Cómo se moverán: coche, andando, bici o transporte público (travelMode: driving | walking | cycling | transit).",
        ...
        "- Si el día ya tiene actividades y hay huecos en el contexto, pregunta si quieren añadir más antes de proponer items extra (salvo que ya hayan pedido «añade», «busca», «rellena»).",
        "- No dupliques visitas ya listadas ese mismo día; sí puedes planificar el mismo lugar en otro día (multi-día o visita distinta: ver vs subir).",
        "- Reglas mixtas andar/bici: elige travelMode cycling o walking según predomine y explícalo en el texto.",
        "- Si piden transporte público (metro, bus, tren), usa travelMode \"transit\"; no lo rechaces.",
        "- Si solo piden comer/cenar cerca de sus visitas (sin reorganizar el día), el JSON debe llevar solo items `restaurant` (1–3), sin visitas ni «Desplazamiento a…».",
      ].join("\n"),
    travel_docs: [
