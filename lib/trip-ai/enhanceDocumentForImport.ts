...
export async function enhanceDocumentForItineraryImport(params: {
  ...
  const prompt = [
    ...
    "sportEvents": [{ "name": "string", "venue": "string|null", "date": "YYYY-MM-DD|null", "time": "HH:MM|null" }]',
    "}",
    "Extrae todos los hoteles, vuelos, partidos deportivos, excursiones y tours visibles. dayCount = nº de días del calendario.",
    "Si detectas partidos deportivos (NBA, NFL, baseball, fútbol), extráelos como flights[] con route=\"partido: [equipo1] vs [equipo2]\" y date/time si aparecen.",
    "Si detectas excursiones de un día (ej. visita a Green Bay, tour a Lambeau Field), inclúyelas en el summary.",
    "",
