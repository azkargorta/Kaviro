...
export async function PATCH(request: Request, { params }: { params: { expenseId: string } }) {
  try {
    ...
    const forbidden = forbidUnlessCanManageExpenses(gate.access, "No tienes permisos para editar gastos.");
...
export async function DELETE(_request: Request, { params }: { params: { expenseId: string } }) {
  try {
    ...
    const gate = await requireTripAccessApi(String(row.trip_id));
    if (!gate.ok) return gate.response;
    const forbidden = forbidUnlessCanManageExpenses(gate.access, "No tienes permisos para borrar gastos.");
    if (forbidden) return forbidden;
