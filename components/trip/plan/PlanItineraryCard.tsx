      ...
      {open && extra > 0 ? (
        ...
        >
          {participants.map((name) => (
            ...
            >
              <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[var(--brand-light)] text-[10px] font-bold text-[var(--brand)]">
                {planParticipantInitials(name)}
              </span>
...
export default function PlanItineraryCard({
  ...
  return (
    <div className="overflow-hidden rounded-3xl border border-slate-200/90 bg-white shadow-[0_4px_20px_rgba(15,23,42,0.07)] dark:border-[#1E293B] dark:bg-[#0F1623] dark:shadow-[0_4px_20px_rgba(0,0,0,0.35)]">
      ...
      <div className="bg-gradient-to-r from-[var(--brand)] to-[var(--brand-hover)] px-3 py-2.5 md:px-5 md:py-4">
        <div className="flex items-center justify-between gap-2 md:items-start md:gap-3">
          <div className="min-w-0 flex-1">
            {destLabel ? (
              <p className="hidden text-xs font-bold uppercase tracking-widest text-white/70 md:block">{destLabel}</p>
            ) : null}
            <p className="truncate text-sm font-extrabold text-white md:mt-0.5 md:text-lg">{tripName}</p>
          </div>
        ...
        <div className="flex items-stretch border-b border-slate-100 bg-slate-50 dark:border-[#1E293B] dark:bg-[#080C14]">
          {canScrollLeft ? (
            <button
              ...
              className="flex w-9 shrink-0 items-center justify-center border-r border-slate-200 text-[var(--brand)] transition hover:bg-white dark:border-[#1E293B] dark:hover:bg-[#0F1623]"
                  ...
                  {tab.date ? (
                    <span
                      className={`text-[10px] font-semibold leading-none ${
                        isActive ? "text-[var(--brand-text)]" : "text-slate-400 dark:text-slate-500"
                      }`}
                    >
                      {tab.date}
                    </span>
          ...
          {canScrollRight ? (
            <button
              ...
              className="flex w-9 shrink-0 items-center justify-center border-l border-slate-200 text-[var(--brand)] transition hover:bg-white dark:border-[#1E293B] dark:hover:bg-[#0F1623]"
