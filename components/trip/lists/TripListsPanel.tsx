                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between sm:gap-3">
                  ...
                  <div className="flex min-w-0 flex-wrap gap-2">
                    ...
                    <button
                      ...
                      className="whitespace-normal rounded-xl border border-slate-200 bg-white px-2.5 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 dark:border-[#334155] dark:bg-[#0F1623] dark:text-slate-200 dark:hover:bg-[#1E293B] sm:px-3"
                      disabled={saving}
                    >
                      Renombrar
                    </button>
                    {/* #14 — Copiar lista como texto */}
                    <button
                      type="button"
                      onClick={() => {
                        if (!itemsApi.items.length) return;
                        const text = [
                          `📋 ${selected.title}`,
                          "",
                          ...itemsApi.items.map((item) =>
                            `${item.is_done ? "✅" : "⬜"} ${item.text}${item.qty ? ` (×${item.qty})` : ""}${item.note ? ` — ${item.note}` : ""}`
                          ),
                          "",
                          "📱 Kaviro · kaviro.app",
                        ].join("\n");
                      ...
                      className="whitespace-normal rounded-xl border border-slate-200 bg-white px-2.5 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 dark:border-[#334155] dark:bg-[#0F1623] dark:text-slate-200 dark:hover:bg-[#1E293B] sm:px-3"
                      disabled={saving || !itemsApi.items.length}
                    >
                      📋 Copiar lista
                    </button>
                    <button
