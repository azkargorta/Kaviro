...
export default function TripHeroMobileShareSheet({ tripId, tripName, destination }: Props) {
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between border-t border-slate-100 px-3 py-2 dark:border-[#1E293B] md:hidden max-md:pl-[max(0.75rem,var(--safe-area-left))] max-md:pr-[max(0.75rem,var(--safe-area-right))]">
        <Link
          href="/dashboard"
          className="inline-flex min-h-9 items-center gap-1.5 rounded-xl border border-slate-200/90 bg-slate-50 px-3 py-1.5 text-[11px] font-bold text-slate-700 transition hover:bg-slate-100 dark:border-[#334155] dark:bg-[#141c2b] dark:text-slate-200"
          aria-label="Volver a Mis viajes"
        >
          <Home className="h-3.5 w-3.5 text-[var(--brand)]" aria-hidden />
          Mis viajes
        </Link>
        <button
          type="button"
          onClick={() => setOpen(true)}
          className="inline-flex min-h-9 items-center gap-1.5 rounded-xl border border-slate-200/90 bg-white px-3 py-1.5 text-[11px] font-bold text-slate-700 shadow-sm transition hover:border-[var(--brand-border)] dark:border-[#334155] dark:bg-[#0F1623] dark:text-slate-200"
          aria-expanded={open}
