...
export default function ShareTodayPlanButton({
  ...
  const heroClass =
    "inline-flex w-full min-h-[44px] items-center justify-center gap-2 rounded-xl border border-white/35 bg-white/15 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-white/25 disabled:opacity-60";
  ...
  return (
    <button
      ...
      title="Abre WhatsApp con el plan de hoy en texto"
    >
      <MessageCircle className="h-4 w-4 shrink-0" aria-hidden />
      {busy ? "Preparando…" : "Enviar plan de hoy por WhatsApp"}
    </button>
  );
}
