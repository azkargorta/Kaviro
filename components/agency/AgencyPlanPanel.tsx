...
import { CheckCircle2, CreditCard, Loader2, Mail } from "lucide-react";
...
export default function AgencyPlanPanel() {
  ...
  return (
    <div className="space-y-6">
      ...
      {!status.active ? (
        <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-950 dark:border-amber-900/40 dark:bg-amber-950/30">
          Tu plan no está activo. Actualiza a Agency Pro o contacta con soporte.
        </div>
      ) : null}
      ...
      {status.canUpgrade ? (
        <div className={`${agencyCardClass} grid gap-6 p-6 md:grid-cols-2`}>
          <div>
            ...
            >
              {checkoutBusy ? <Loader2 className="h-4 w-4 animate-spin" aria-hidden /> : <CreditCard className="h-4 w-4" aria-hidden />}
              Pasar a Agency Pro
            </button>
