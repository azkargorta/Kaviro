      ...
      <main className="mx-auto max-w-6xl px-4 pb-16 pt-8 sm:px-6 sm:pt-10">
        ...
        {data.steps && data.steps.length > 0 ? (
          <section className="mt-14" aria-labelledby="steps-heading">
            <Reveal variant="fade">
              <h2 id="steps-heading" className="text-center text-2xl font-extrabold text-slate-900 dark:text-white">
                Cómo empezar
              </h2>
            </Reveal>
            <ol className="mt-8 grid gap-4 md:grid-cols-3">
              {data.steps.map((step, idx) => (
                <Reveal key={step.title} variant="scale" delay={(idx % 3) as 0 | 1 | 2}>
                  <li className="rounded-2xl border border-[var(--brand-border)] bg-[var(--brand-light)] p-5 dark:bg-[var(--brand-light)]/20">
        ...
        <Reveal variant="fade" className="mt-14">
          <section className="rounded-3xl border border-[var(--brand-border)] bg-[var(--brand-light)] px-6 py-10 text-center dark:bg-[var(--brand-light)]/15 md:px-10">
            ...
            <h2 className="mt-3 text-xl font-extrabold text-slate-900 dark:text-white md:text-2xl">
              Empieza a organizar tu próximo viaje
            </h2>
            <p className="mx-auto mt-2 max-w-lg text-sm text-slate-600 dark:text-slate-300">
              Crea tu espacio en {APP_NAME}, invita al grupo y ten plan, gastos y documentos en un solo sitio.
            </p>
              ...
              {related.map((page) => (
                ...
                >
                  <p className="text-sm font-bold text-slate-900 group-hover:text-[var(--brand)] dark:text-white">
                    {page.hero.h1.split(":")[0]}
                  </p>
                  <p className="mt-1 text-xs leading-relaxed text-slate-500 dark:text-slate-400">
                    {page.metadata.description.slice(0, 100)}…
                  </p>
                </Link>
              ))}
            </div>
